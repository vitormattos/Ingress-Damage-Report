...
class IngressSentinel {
	...
	public function __construct($data_connection) {
		...
		if(!file_exists('db.sqlite3')) {
			throw new Exception('Run migration for create sqlite file');
		}
		if( empty($_SERVER['REMOTE_ADDR']) and !isset($_SERVER['HTTP_USER_AGENT']) and count($_SERVER['argv']) > 0) {
			$this->cli = true;
		}
		$this->db = new \PDO('sqlite:db.sqlite3');
		$this->imap = new Imap($data_connection);
		if(!$this->imap) {
	...
	private function savePortal($data) {
		...
		if(!$portal = $portal->fetch(\PDO::FETCH_ASSOC)) {
			$portal['name'] = $data['portal_name'];
			$portal['link'] = $data['portal_link'];
			$portal['address'] = $data['portal_address'];
			...
			$values['name'] = $data['portal_name'];
			$values['link'] = $data['portal_link'];
			$value['address'] = $data['portal_address'];
